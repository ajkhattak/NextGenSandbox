...
class TestSaveDivideOutput(unittest.TestCase):
    def test_configure_removes_stale_nexus_netcdf(self):
        plugin = SaveData()

        with tempfile.TemporaryDirectory() as temp_dir:
            workdir = Path(temp_dir)
            stale_output = workdir / "formulation_default_nexuses.nc"
            stale_output.write_text("incomplete output")

            plugin.ngen_cal_model_configure(
                SimpleNamespace(plugin_settings={}, workdir=workdir)
            )
    ...
    def test_saves_only_outputs_from_best_iteration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (workdir / "formulation_default_nexuses.nc").write_text(
                "iteration 0"
            )
            ...
            self.assertFalse(
                (workdir / "formulation_default_nexuses.nc").exists()
            )
